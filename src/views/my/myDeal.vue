	...
	<div class="myDeal">
						...
						<div class="list">
							<div class="item" v-for="item in list2" :key="item.id">
								<div class="flex">
									...
									<div class="timeBox">
										匹配时间 {{ item.machingTime.substring(0,19) }}
									</div>
									<div class="timeBox">
							...
							<div class="item" v-for="item in list3" :key="item.id">
								<div class="flex">
									...
									<div class="timeBox" v-if="item.coinReleaseTime">
										完成时间 {{ item.coinReleaseTime.substring(0,19) }}
									</div>
									<div class="timeBox">
										订单编号 <span class="copy" @click="handleCopy(item.id,$event)">{{ item.id }}</span>
									</div>
		...
		<van-action-sheet v-model="showSellerDetailModel" title="订单详情">
			...
			<div class="detailBox" v-if="showSellerDetailLoading==false&&detail4sellerInfo">
				...
				<div class="line">
					<span class="label">挂单时间</span>
					<span class="value">{{detail4sellerInfo.hangBillTime}}</span>
				</div>
				<div class="line">
					<span class="label">匹配时间</span>
					<span class="value">{{detail4sellerInfo.machingTime}}</span>
				</div>
				<div class="line">
		...
		<van-action-sheet v-model="showBuyerDetailModel" title="订单详情">
			...
			<div class="detailBox" v-if="detail4buyerInfo">
				...
				<div class="line">
					<span class="label">挂单时间</span>
					<span class="value">{{detail4buyerInfo.hangBillTime}}</span>
				</div>
				<div class="line">
					<span class="label">匹配时间</span>
					<span class="value">{{detail4buyerInfo.machingTime}}</span>
				</div>
				<div class="line">
				...
				<div class="line" v-if="detail4buyerInfo.status==12">
					<span class="label">上传凭证</span>
					<span class="textCenter">若遇到图片上传不了的解决办法：换浏览器操作，UC/QQ浏览器优先</span>
					<span class="valuePic">
						<i class="iconfont iconfont-upload-pic"></i>
				...
				<div class="margT10" v-if="imgData&&(detail4buyerInfo.status==12)">
				<img class="selectedImg" :src="imgData"/>
				</div>
				...
				<div class="margT10" v-if="detail4buyerInfo.imgUrl&&(detail4buyerInfo.status==4||detail4buyerInfo.status==5)">
		...
		<van-action-sheet v-model="showSureCancelTransactionModel4seller" title="温馨提示">
			<div class="cancelSellTip">
				<div class="tipText2">若买方在120分钟内没付款，且联系无果，卖方可取消交易。锁定交易后务必在30分钟内付款。</div>
		...
		<van-dialog v-model="showTipModel" title="问题小帮手" confirmButtonText="好的">
			<div class="paddingWing f-12 lineHeight tip4model2 textJustify">
				<div class="textIndent">
					单子匹配后，请卖家主动提示买家去查看订单信息，然后耐心等待120分钟，若买方在120分钟内没付款，也没锁定交易，卖方可取诉讼申请消交易。单子匹配后，一旦锁定交易后，务必在30分钟内付款。（注：买方若是要通过微信所绑定的手机号转账，请卖方在微信中的'<b>支付-支付管理</b>'中开通'<b>允许通过手机号向我转账</b>'的功能）。
				</div>
			</div>
		</van-dialog>
	...
	export default {
		data() {
			return {
				...
				tipText:"<b class='textBold'>温馨提示：</b><br>单子一旦匹配，请卖方务必【发送短信提醒】，然后耐心等待120分钟，若买方在120分钟内没付款，也没锁定交易，卖方可申诉申请取消交易。<br>（注：买方若是要通过微信所绑定的手机号转账，请卖方预先在微信中的【支付-支付管理】中开通【允许通过手机号向我转账】的功能）",
				tipText4AppointDeal: "",
				activeName: "pay",
				pageCount: 1000,
				totalItems: 10000,
			...
			if(_this.$route.query.mobilePhone){
				...
				if(_this.$route.query.dealType==1){
					if(_this.$route.query.isSelf) {
						...
						_this.smsContent = `【${_this.$api.projectName}】我所转让的${_this.$route.query.num}个矿石已经匹配到了您，请在“个人中心--我的交易--待付款”的订单详情中查看并及时完成交易。`;
					}else{
		...
		methods: {
						...
						img.onload = function() {
							...
							_this.$ajax.ajax(_this.$api.updateTransactionImgUrlById, 'POST', params, function(res) {
								if (res.code == _this.$api.CODE_OK) {
									...
									if(_this.detail4buyerInfo.status==12){
										//让卖家上传发短信提醒的照片，不用短信提醒买家
									}else{
										_this.smsContent = `【${_this.$api.projectName}】我的付款凭证已上传，请在“个人中心--我的交易--待收款”的订单详情中查看。`;
										_this.setSendSmsHref(_this.mobilePhone,_this.smsContent);
									}
			...
			lockTransactionBtn(item){
				...
				Dialog.confirm({
				  title: '确认信息',
				  confirmButtonText:'确定',
				  message: '锁定交易后务必在20分钟内完成付款，您是否确定要锁定交易？'
				}).then(() => {
					// on confirm
					let params = {
						/* userId:  _this.userId, */
				  ...
				  _this.$ajax.ajax(_this.$api.updateTransactionStatusById, 'POST', params, function(res) {
				  	if (res.code == _this.$api.CODE_OK) {
				  		...
				  		if(res.data==1){
							...
							_this.smsContent = `【${_this.$api.projectName}】您好，我并未收到款，请在“个人中心--我的交易--待付款”的订单详情中上传付款凭证。`;
							_this.setSendSmsHref(_this.mobilePhone,_this.smsContent);
							_this.showBuyerDetailModel = false;
							_this.onLoad4();
						}
				  	...
				  	if (res.code == _this.$api.CODE_OK) {
				  		...
				  		if(res.data==1){
							...
							if(_this.type == 0 || _this.type == 1 || _this.type == 3){
								...
								_this.mobilePhone = _this.detail4sellerInfo.mobilePhone;
								_this.smsContent = `【${_this.$api.projectName}】所匹配的${item.num}个矿石已付款，请确认收款，并在“个人中心--我的交易--待收款”的订单详情中确认收款并释放矿石。HPC帮扶链感恩有您的支持!`;
								_this.setSendSmsHref(_this.mobilePhone,_this.smsContent);
								//发送短信提示end
								_this.showSellerDetailModel = false;
