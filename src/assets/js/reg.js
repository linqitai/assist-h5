...
export default{
	...
	passwordHint:"请填写6~16位登录密码，由'字母或数字或._'组成",
	password2Hint:'请填写与上面一致的登录密码',
	safePassword:/^[A-Za-z0-9._]{1,16}$/,
	safePasswordHint:"安全密码不超过16位，由'字母或数字或._'组成",
	phone:/^(1[3-9])\d{9}$/,
	phone2:/^[0-9]{6,11}$/,
	phoneHint:'请填写登录手机号',
	validateCode:/^[A-Za-z0-9]{6}$/,
	validateCodeHint:'请填写6位短信验证码',
	shareCode:/^[A-Za-z0-9]{7,7}$/,
	shareCodeHint:'请填写7位推荐码',
	alipay:/^(?:\w+\.?)*\w+@(?:\w+\.)+\w+|\d$/,
	alipayHint:'请正确填写支付宝号',
	weichat:/(^[a-zA-Z]{1}[-_a-zA-Z0-9]{5,19}$)|(^[0-9]{6,11}$)/,
	weichatHint:'请正确填写以字母开头的微信号',
	idCard:/(^\d{12,18}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/,
