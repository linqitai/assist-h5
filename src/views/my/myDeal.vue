...
<template>
	<div class="myDeal">
		...
		<van-action-sheet v-model="showSureCancelTransactionModel4buyer" title="温馨提示">
			<div class="cancelSellTip">
				<div class="tipText2">为了避免买方随意取消交易而导致扰乱市场的现象，经平台研究决定，买方主动取消交易或因超时未打款而被动取消交易，要扣0.5个贡献值。若卖方实名信息不符或者有其他问题，请向平台诉讼反馈问题，让客服来处理单子。</div>
			</div>
			<div class="sureAppointBtnBox">
				  	...
				  	if (res.code == _this.$api.CODE_OK) {
				  		...
				  		if(res.data==1){
							...
							if(_this.type == 0 || _this.type == 1 || _this.type == 3){
								...
								_this.mobilePhone = _this.detail4sellerInfo.mobilePhone;
								_this.smsContent = `【${_this.$api.projectName}】所匹配的${item.num}个矿石已付款，请确认收款，并在“我的--我的单子--待收款”的订单详情中确认收款并释放矿石。每笔交易的手续费中都会有20%~50%的帮扶券存入少年儿童帮扶基金池，HPC帮扶链感恩有您的支持!。`;
								_this.setSendSmsHref(_this.mobilePhone,_this.smsContent);
								//发送短信提示end
								_this.showSellerDetailModel = false;
